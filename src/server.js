...
app.get('/api/config', (req, res) => {
  res.json({
    storeName: setting('store_name', 'Mariane Moreira Concept'),
    slogan: setting(
      'slogan',
    ...
    whatsapp: setting('whatsapp', '5511945921719'),
    instagram: setting('instagram', ''),
    primaryColor: setting('primary_color', '#2B2023')
  });
});
...
app.get('/api/products', (req, res) => {
  ...
  args.push(Math.min(Number(limit) || 60, 100));
...
app.get('/api/admin/dashboard', auth, (req, res) => {
  ...
  res.json({
    ...
    coupons: count(
      'SELECT COUNT(*) n FROM coupons WHERE active = 1'
    ),
...
app.get('/api/admin/products/:id', auth, (req, res) => {
  res.json(productFull(req.params.id));
});
...
app.post('/api/admin/products', auth, (req, res) => {
  try {
    ...
    `).run(
      ...
      Number(b.price),
      b.promo_price
        ? Number(b.promo_price)
        : null,
      b.category_id || null,
      b.subcategory || '',
      b.brand || 'Mariane Moreira',
      b.sku || null,
      Number(b.stock_min || 2),
      b.material || '',
      b.composition || '',
      b.measurements || '',
      Number(b.weight || 0),
    ...
    res.status(201).json(productFull(id));
  } catch (e) {
    res.status(400).json({
      error: e.message
    });
...
app.delete('/api/admin/campaigns/:id', auth, (req, res) => {
  db.prepare(
...
app.get('/api/admin/settings', auth, (req, res) => {
  res.json({
    ...
    hero_banner: setting('hero_banner')
  });
});
...
app.post('/api/orders', (req, res) => {
  try {
    ...
    for (const i of b.items) {
      const p = productFull(i.productId);
      ...
      const v = p.variants.find(
        x => String(x.id) === String(i.variantId)
      );

      if (
        !v ||
        v.stock < Number(i.quantity)
      ) {
        throw new Error(
